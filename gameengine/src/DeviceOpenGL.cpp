...
void DeviceOpenGL::vertexAttribPointer( const VertexData& meta )
{
    ...
    bindBuffer( BufferTypes::VERTEX_ARRAY, meta.VAO->getId() );
    bindBuffer( BufferTypes::ARRAY_BUFFER, meta.VBO->getId() );
...
void DeviceOpenGL::deleteBuffer( BufferTypes bufferType, std::uint64_t& id )
{
    if( !CUL::CULInterface::getInstance()->getThreadUtils().getIsCurrentThreadNameEqualTo( "RenderThread" ) )
    {
        CUL::Assert::simple( false, "NOT IN THE RENDER THREAD." );
