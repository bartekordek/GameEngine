...
void SDL2WrapperImpl::init( const WinData& wd, const CUL::FS::Path& configPath )
{
    ...
    m_logger->logInfo( "Available cpu cores: %d", SDL_GetCPUCount() );
    ...
    {
        const String rendererName = m_configFile ? m_configFile->getValue( "RENDERER" ).getString() : CUL_STR( "" );
...
void SDL2WrapperImpl::createKeys()
{
    ...
    for( size_t i = 0; i < keyCount; ++i )
    {
        ...
        if( !keyName.empty() )
        {
            ...
            m_logger->logInfo( "Name: %s, id: %d", keyName.getUtfChar(), i );
        }
    }
